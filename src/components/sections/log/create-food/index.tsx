...
import { createFood, createFoodAndLog } from "@/app/create-food/action";
...
const CreateFood = ({ onClose, hour }: Props) => {
    ...
    <div className="flex flex-col h-full bg-[#1c1c1e] text-white relative">
      ...
      {step === 3 && (
        <Step3Nutrition
          ...
          total_fat={total_fat} setTotalFat={setTotalFat}
          saturated_fat={saturated_fat} setSaturatedFat={setSaturatedFat}
          total_carbs={total_carbs} setTotalCarbs={setTotalCarbs}
          fiber={fiber} setFiber={setFiber}
          sugars={sugars} setSugars={setSugars}
          total_protein={total_protein} setTotalProtein={setTotalProtein}
      ...
      <div className="flex justify-between items-center px-4 py-5">
        ...
        {step < 3 ? (
          ...
          <div className="flex gap-2">
            <button onClick={handleCreate} className="flex items-center gap-1 bg-[#3a3a3c] text-white font-semibold text-sm px-5 py-3 rounded-full">
              Create ✓
            </button>
            <button onClick={handleCreateAndLog} className="flex items-center gap-1 bg-white text-black font-semibold text-sm px-5 py-3 rounded-full">
              Create & Add ✓+
            </button>
          </div>
        )}
